...
// Copyright (c) 2014 choic Developers
...
//uint256 hashGenesisBlock("0xa9df6ebcb29fc27d67c3588f574846476f8fba66cccdd7e1ac72885e9f5723e5");
uint256 hashGenesisBlock("0xace08d5e4f7fdb0135e3ae33c7fd01dab4eec0f1288eaa630f71f014a2049257");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Choic: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Choic Signed Message:\n";
...
{
    // Choic: IsDust() detection disabled, allows any valid dust to be relayed.
...
//static const int64 nTargetTimespan = 3*60*60;//3 * 60 * 60; // Choic:every 3 hours
//static const int64 nTargetSpacing =  3*60; // Choic:3 minutes
//static const int64 nInterval = nTargetTimespan / nTargetSpacing;

static int64 nTargetTimespan = 3*60*60;//2 * 60 * 60; // Choic:every 2 hours
static int64 nTargetSpacing =  30; // Choic:1 minutes
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if(nHeight<=32000)
    {
        nTargetTimespan = 3*60*60;//3 * 60 * 60; // Choic:every 3 hours
        nTargetSpacing =  20; // Choic:20 s
        nInterval = nTargetTimespan / nTargetSpacing;
    }
    else
    {
        nTargetTimespan = 3*60*60;//3 * 60 * 60; // Choic:every 3 hours
        nTargetSpacing =  3*60; // Choic:3 minutes
        nInterval = nTargetTimespan / nTargetSpacing;
    }*/
    ...
    /*else if(nHeight<=16000)
    {
        nSubsidy=(18+(7-nHeight/1000))*100;
        //7001---8000块 每块1800
        //24001---25000 每块100
    }*/
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Choic: This fixes an issue where a 51% attack can change difficulty at will.
...
{
    // Choic: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "start 20140329";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1396059917 ;//2014-3-29 10:25:17 //1391830000;//2014-2-8 11:26:40
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 5922307;//
        ...
        assert(block.hashMerkleRoot == uint256("0x22fc2709d760d4a35415dc4974a5eed439f4be37fef6a7256c8fdd32e188d981"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Choic: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ChoicMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ChoicMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ChoicMiner(CWallet *pwallet)
{
    printf("ChoicMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("choic-miner");
    ...
    try { loop {
        ...
        printf("Running ChoicMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ChoicMiner, pwallet));
}
